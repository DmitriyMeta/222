# Данный репозиторий создан для отслеживания личных достижений и результатов. Здесь скоплены разного рода проекты на языках СИ ++, Python. В будущем будут добавлены ссылки на доп. проекты, перенести которые на гит - трудоемкая задача.
